...
public class Pawn {
  ...
  private static boolean isValidDiagonalMove(Optional<Piece> target, Piece piece) {
		...
		/*
		 * return F.pipe( Optional<Piece>::get, F.ifElse( p -> p.getColor() ==
		 * piece.getColor(), __ -> false, F.pipe( p -> new SimpleEntry<>(
		 * Position.computeXOffset(p.getX(), piece.getX()),
		 * Position.computeYOffset(p.getY(), piece.getY()) ), F.ifElse( __ ->
		 * piece.getColor() == Color.WHITE, o -> Math.abs(o.getKey()) == 1 &&
		 * o.getValue() == 1, o -> Math.abs(o.getKey()) == 1 && o.getValue() == -1 ) ) )
		 * ).apply(target);
		 */
	  return true;
  }
